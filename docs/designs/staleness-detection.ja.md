...
source_refsフィールドが定義されていないドキュメント。ソースコードとの対応付けがないため、陳腐化の判定ができない。

### stale

少なくとも1つのsource_refのハッシュが`last_synced_hash`と異なる状態。参照先のソースコードが変更されており、ドキュメントの更新が必要な可能性がある。

### untracked

source_refsが定義されているが、参照先のファイルが存在しない状態。ファイルの移動・削除・パスの誤りが原因として考えられる。
